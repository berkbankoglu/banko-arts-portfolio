...
export default function BankoArts() {
  ...
  const goPage = (p) => {
    // Works artık ayrı sayfa — tam portfolyo galerisi
    if (p === 'works') { window.location.href = '/works'; return; }
    const map = { about:'section-services', services:'section-services', contact:'section-contact' };
    const el = document.getElementById(map[p] || 'section-works');
    if (el) window.lenis?.scrollTo(el, { offset: -20, duration: 1.6 });
  };

  // /works sayfasından #services gibi hash ile dönüşlerde ilgili bölüme kay
  useEffect(() => {
    const h = window.location.hash.replace('#', '');
    if (!h) return;
    const map = { services:'section-services', about:'section-services', contact:'section-contact' };
    const el = document.getElementById(map[h]);
    if (el) setTimeout(() => window.lenis?.scrollTo(el, { offset: -20, duration: 1.2 }), 150);
  }, []);
  ...
  return (
    <div style={{ minHeight:'100vh', background:'var(--bg)', color:'var(--black)', paddingLeft:'clamp(52px, 7vw, 160px)', paddingRight:'clamp(52px, 7vw, 160px)' }}>
      ...
      <section id="section-works" style={{ borderTop:'1px solid var(--sep)' }}>
        ...
        {/* Tam portfolyo artık /works sayfasında */}
        <div style={{ display:'flex', justifyContent:'center', padding:'0 20px 96px' }}>
          <a href="/works"
            style={{ display:'inline-flex', alignItems:'center', gap:12, padding:'18px 44px',
              border:'1px solid var(--black)', fontSize:13, fontWeight:600, letterSpacing:'0.1em',
              textTransform:'uppercase', color:'var(--black)', transition:'background 0.25s ease, color 0.25s ease' }}
